/*angular.module('eits-timepicker', []).directive("timepickerjquery", function($timeout) {
    return {
        scope: {images: '='},
        template: '',
...
angular.module('eits-timepickerPop', [ 'ui.bootstrap'])
    ...
    .directive('timepickerPop', function($document, timepickerState) {
        return {
            ...
            template : "<input type='text' onkeyup='denyLetters(this)' class='form-control' ng-model='inputTime' ng-disabled='disabledInt' time-format show-meridian='showMeridian' ng-focus='open()' />"
...
function denyLetters(input){
    $(input).val($(input).val().replaceAll("[^0-9:]", ''));
}